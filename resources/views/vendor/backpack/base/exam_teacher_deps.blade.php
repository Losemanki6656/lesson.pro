...
@section('header')
    <div class="container-fluid">
        <h2>
            <span class="text-capitalize"> {{ $manag_name }} - {{$org_name}} - {{$user_name}} </span>
            <small id="datatable_info_stack"> Бўлимлар кўрсаткичлари </small>
        </h2>
    </div>
@endsection
                ...
                @if (count($a))
                    @foreach ($a as $key => $value)
                        <tr>
                            ...
                            <td>
                                <a type="button" href="{{ route('exam_teacher_dep_positions',[
                                    'org_id' => $org_id,
                                    'manag_id' => $manag_id,
                                    'user_id' => $user_id,
                                    'dep_id' => $value['id']
